...
It does so throughout the script via a common method of client-side deobfuscation implemented by scam sites: array rotation obfuscation.
...
We can now approach dissecting the obfuscated array unshuffler. Here's an example of one such unshuffler (We've taken the liberty of deobfuscating it and converting it to Python for readbility and general mental health):
