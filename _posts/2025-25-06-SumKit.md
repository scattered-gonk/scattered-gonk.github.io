...
<b> Visit the scam site at your own risk. It is a phishing site, and any interaction with it inherently carries a high level of risk -- we at SqueakyFlotilla are not liable for any information stolen or data compromised, should individual users seek to experiment with the site. </b>
...
```
in order to retrieve a token which will be assigned to the victim. The token is only returned under the condition that the connecting browser's user agent is indicative of a mobile device.
